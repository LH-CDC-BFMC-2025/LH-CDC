...
class LaneDetection:
    ...
    def process_image(self, frame):
        ...
        steer_angle = -(lpos + rpos - self.width) / 5
        return (lpos, rpos), frame, [left_slope, right_slope], steer_angle
...
if __name__ == '__main__':
    cap = cv2.VideoCapture('X_Lane_Detect/video/navin_testing_1.mkv')
    time.sleep(3)
    ...
    pwm = GPIO.PWM(servo_pin, 50)  # Tần số 50Hz cho servo
    pwm.start(0)

    def set_servo_angle(angle):
        duty = (angle / 18) + 2  # Công thức chuyển đổi góc thành chu kỳ PWM
    ...
    while cap.isOpened():
        _, frame = cap.read()
        (lpos, rpos), frame, slopes, steer_angle = lane_detector.process_image(frame)
        steering.draw_steer(frame, steer_angle)

        # Chuyển đổi góc lái thành góc servo
        servo_angle = int((steer_angle + 90) * 180 / 180)
        set_servo_angle(servo_angle)

        print(f'Steer Angle: {steer_angle:.2f} | Servo Angle: {servo_angle}')
