...
if __name__ == '__main__':
    ...
    while True:
        ...
        if not ret:
            break
        pos, frame, slope, steer_angle = lane_detector.process_image(image)
        steering.draw_steer(frame, steer_angle)

        # Tính toán góc cho servo
        servo_angle = int((steer_angle + 90) * 180 / 180)  # Chuyển đổi từ -90 đến 90 sang 0 đến 180
        print(f'Steering Angle: {steer_angle:.2f} | Servo Angle: {servo_angle}')
